...
var ExportSelector = Class.create( {
    ...
    initialize: function() {
        ...
        typeListElement.insert(_addTypeOption(false, "Gel Pedigree Model", "gelJSON"));
        ...
        var configListElementGEL = new Element('table', {"id": "gelOptions", "style": 'display:none'});
        configListElementGEL.insert(_addConfigOption(true,  "export-options", "import-config-label", "All data", "all"));
        ...
        dataSection3.insert(promptConfig).insert(configListElementGEL).insert(configListElementJSON).insert(configListElementPED);
    ...
    _onExportStarted: function() {
        ...
        } else if (exportType == "gelJSON") {
            var privacySetting = $$('input:checked[type=radio][name="export-options"]')[0].value;
            var exportString = PedigreeExport.exportAsGELPedigree(editor.getGraph().DG, privacySetting);
            var fileName = patientDocument + ".json";
            var mimeType = "application/json";
